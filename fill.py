...
'''TODO: fill with matrix factorization,  change Vote_Matrix '''

Path = '/home/yangying/mq_data/'
Cluster_Result = Path + '/fulldata_xmeans.csv'
...
def fill(rule,sdf,method = 'rake',threshold = 0.6):
	'''
	to call this function
	rule: dict-like object
	sdf: your specific dataframe
	...
	df = pd.read_csv(Cluster_Result)
	USER_NUM,col = df.as_matrix().shape
	...
	if number == 0:
		...
		count = fill_with_col_ap(matrix,querylist,edge,data_1,sim,datatype = 'binary')
		#count = fill_with_col_ap(matrix,querylist,edge,data_1,sim,datatype = 'float')
...
def fill_with_col_ap(m,querylist,edge,data_1,sim,datatype='binary'):
	'''
	fill the matrix with ap,convergence
	iterate with col
	faster.
	datatype options: 'binary'--> yes/no
	...
	tmp_change = 0
	vd = util.ValueDict(edge,datatype)
		...
		for pair in match:
			...
			while j < row:
				...
				if datatype == 'binary':
					...
					print 'wrong value with',value
					sys.exit()
				if ans == None or vd.isSame(tmp[j][node],ans) or tmp[j][node] == ans: 
					j += 1
					continue
				tmp[j][node] = ans
				changed += 1
